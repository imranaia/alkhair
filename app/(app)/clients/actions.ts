...
const clientSchema = z.object({
  ...
  landmark: z.string().trim().min(1, "Landmark is required.").max(150),
  groupName: z.string().trim().min(1, "Group is required.").max(80),
  businessType: z.string().trim().min(1, "Trade / business is required.").max(80),
  businessLocation: z.string().trim().min(1, "Business location is required.").max(120),
  ...
  neighborRelativePhone: z.string().trim().min(1, "Neighbor's / relative's phone is required.").max(30),
  shopOwner: z.coerce.boolean(),
  rentingShop: z.coerce.boolean(),
  gpsPhotoVerified: z.coerce.boolean(),
  gpsTimeVerified: z.coerce.boolean(),
  experienceYears: z.coerce.number().int().nonnegative({ message: "Experience in business is required." }),
  customerType: z.enum(["walk_in", "marketing"]),
});
...
export async function createClientAction(_prevState: ClientFormState, formData: FormData): Promise<ClientFormState> {
  ...
  const client = await createClient({
    ...
    nin: parsed.data.nin,
    neighborRelativePhone: parsed.data.neighborRelativePhone,
